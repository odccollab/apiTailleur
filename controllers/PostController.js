...
class PostController {
  static async signalPost(req, res) {
    ...
    try {
      ...
      if (
        post.signale.some(
          (signalement) => signalement.idLikerD.toString() === userId
        )
      ) {
      ...
      if (post.signale.length > 2) {
        ...
        UserController.addNotification(userId, {
          message:
            "Votre post a été supprimé en raison de trop de signalements",
          type: "post",
          idPost: postId,
          date: new Date(),
        });
  ...
  static async getAllStatus(req, res) {
    try {
      const status = await Post.find(
        {},
        { _id: 1, contenu: 1, contenuMedia: 1, createdAt: 1 }
      );
      res.json(status);
    } catch (err) {
      console.error(err.message);
  ...
  static async incrementViews(req, res) {
    ...
    try {
      ...
      if (post.viewersIds.includes(req.id)) {
        return res.json({ views: post.viewersIds });
      }

      // Incrémenter le nombre de vues et ajouter l'ID de l'utilisateur
      const updatedPost = await Post.findByIdAndUpdate(
        postId,
        { $inc: { views: 1 }, $addToSet: { viewersIds: req.id } },
        { new: true }
      );
  ...
  static async addComment(req, res) {
    ...
    try {
      ...
      if (!text|| !text.trim()) {
        return res
          .status(400)
          .json({ message: "Le texte du commentaire ne peut pas être vide" });
      }
  ...
  static async handleLikeDislike(req, res) {
    ...
    try {
      ...
      const existingEntry = post.likesDislikes.find(
        (entry) => entry.idLikerD == userId
      );
      if (existingEntry) {
        if (type === "neutre") {
          post.likesDislikes = post.likesDislikes.filter(
            (entry) => entry.idLikerD.toString() != userId
          );
        } else {
    ...
    try {
      ...
      for (const userId of userIds) {
        await User.findByIdAndUpdate(userId, {
          $push: { sharedPosts: post._id },
        });
        UserController.addNotification(userId, {
          type: "story",
          message: `${req.prenom} ${req.nom} vous a partager un post `,
        });
      }
