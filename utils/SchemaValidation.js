...
const validationSchemas = {
  register: Joi.object({
    ...
    post: Joi.object({
      ...
      contenuMedia: Joi.array()
        .items(
          Joi.string()
            .uri()
            .regex(/\.(jpeg|jpg|gif|png|mp4|mov)$/)
        )
        .optional()
        .messages({
          "array.base": "Le contenu média doit être un tableau",
          "array.includes":
            "Le contenu média doit contenir des URLs valides de vidéos ou d'images",
        })
    ...
    comment: Joi.object({
      text: Joi.string().optional().messages({
        "string.base":
          "Le texte du commentaire doit être une chaîne de caractères",
